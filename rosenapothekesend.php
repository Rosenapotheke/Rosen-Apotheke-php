...
/* Developed by Moka, @ad7863, mgakashim.com, me@mgakashim.com */
	...
	require_once('recaptchalib.php');
	  $privatekey = "6LefTt4SAAAAALlGhosJW3POt8L0OvJ7We2I9eEl"; // enter your private key here
  ...
  if (!$resp->is_valid) {
    ...
    die("Leider gibt es Fehler bei Ihren Angaben.<br>Bitte versuchen Sie es nochmal."); // kill program and return error message
  	...
  	if(isset($_POST['contactFormSubmitted'])) {
		...
		<link href="http://www.rosen-apoth.de/assets/css/bootstrap.css" rel="stylesheet">
		<link href="http://www.rosen-apoth.de/assets/css/rosenapotheke.css" rel="stylesheet">
		
		</head>
		<body>
		<h3 class="blau">www.rosen-apoth.de</h3>
		<h3 class="blau">Nachricht vom ' . $datum . " - " . $uhrzeit . ' Uhr</h3>
		<p class="lead">Antwort <strong>Nummer ' . $nummer . '</strong> auf Kontaktformular Rosenapotheke</p>
		...
		<a href="mailto:' . $email . '?subject=Rosenapotheke Leutkirch | Ihre Kontaktaufnahme vom ' . $datum .'"    role="button" class="btn btn-large btn-primary">Antwortschreiben verfassen</a>
		</html>';		
...
//		$recipient = "windaktiv@hhhaesler.org"; "haesler@hhhaesler.org";
		...
		$subject = "<Kontaktformular Rosenapotheke> | Nachricht von  $vorname $name";
		// 'From' mail header
...
//		$mailheader = "From: $email \r\n"; 
		...
		echo "<script>$('#nachricht').val(''); $('#vorname').val(''); $('#name').val(''); $('#ort').val(''); $('#email').val(''); $('#telefon').val(''); javascript:Recaptcha.reload(); $('form#rosenapotheke').fadeOut(); 
		$('div#fehler').removeClass('fehlermeldung');
		$('div#fehler').addClass('erfolg');
		</script>";
